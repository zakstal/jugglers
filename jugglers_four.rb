...
class Circuit < Skilz
  ...
  def add_juggler_to_circuit(juggler)
    if !@jugglers[5].nil?
      replace_shittiest_if_not_shittier(juggler)
    else
      @jugglers[5] = juggler
      @jugglers = @jugglers.sort_by { |juggler| juggler.nil? ? 200 :  -juggler.first_choice[1] }
    end
  end
  ...
  def replace_shittiest_if_not_shittier(juggler)
    if @jugglers.last.dot_value > juggler.dot_value
      ...
      @jugglers[5], juggler = juggler, @jugglers[5]
      juggler.remove_choice
      @jugglers = @jugglers.sort_by { |juggler| -juggler.first_choice[1] }
    end
  end
  ...
  class Course
    ...
    def add_juggler_to_circuit(juggler)
      circuit_number, dot_v = juggler.first_choice[0]
